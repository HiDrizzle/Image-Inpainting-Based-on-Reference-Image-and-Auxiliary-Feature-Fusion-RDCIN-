...
def SampleEdgeLineLogits(model, context, mask=None, iterations=1, device='cuda', add_v=0, mul_v=4):
    [img, grad, line, grad_re, Edge, Edge_re] = context
    img = img.to(device)
    grad = grad.to(device)
    grad_re = grad_re.to(device)
    line = line.to(device)
    mask = mask.to(device)
    img = img * (1 - mask)
    grad = grad * (1 - mask)
