...
from Reinpainting.src.models.inception import InceptionV3
import torch.nn as nn
import lpips


def get_activations(images, model, batch_size=16, dims=2048,
...
def calculate_activation_statistics(images, model, batch_size=4,
...
def get_inpainting_metrics(src, tgt, logger, fid_test=True):
    ...
    for p1, p2 in tqdm(zip(input_paths, output_paths)):
        ...
        ssim_ = compare_ssim(gray_image1, gray_image2, data_range=255)
        psnrs.append(psnr_)
        ssims.append(ssim_)
        mses.append(mse_)
        maes.append(mae_)
